...
import { toToolItem } from '@/api/mappers'
...
function ToolNameCell({ name, maker, model, note }: {
  name: string
  maker: string
  model: string
  note: string
...
const addDefaults: ToolFormValues = {
  name: '',
  maker: '',
  model: '',
  spec: '',
...
export default function Tools() {
  ...
  const [op, setOp] = useState<{ mode: 'lend' | 'return'; item: ToolItem } | null>(null)
  ...
  /** 从后端拉取工具列表（全量，本地过滤/分页） */
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await itemsApi.list({ type: 'tool', page_size: 1000 })
      setData(res.items.map(toToolItem))
    } catch (err) {
  ...
  const columns: TableProps<ToolItem>['columns'] = [
    {
      title: '编号',
      dataIndex: 'id',
      key: 'id',
    ...
    },
    {
      ...
      render: (_v: string, r) => (
        <ToolNameCell name={r.name} maker={r.maker} model={r.model} note={r.note} />
      ),
    },
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0" style={{ height: '100%' }}>
      <PageToolbar
        searchPlaceholder="搜索工具编号、名称、型号..."
        searchValue={keyword}
        onSearch={setKeyword}
